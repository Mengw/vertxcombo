...
public class ChainedFileHandler {
	...
	public void pump(){
		vertx.fileSystem().open(this.outfile, null, false, true, true, true,new AsyncResultHandler<AsyncFile>(){
			@Override
	...
	private void pumpOne(final AsyncFile out){
		...
		if(infstr == null){
			...
			vertx.fileSystem().open(infstr, null, true, false, false, false,new AsyncResultHandler<AsyncFile>(){
				@Override
				public void handle(AsyncResult<AsyncFile> event) {
					if(event.succeeded()){
